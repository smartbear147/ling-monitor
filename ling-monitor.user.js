...
// @version      1.0.0
// @description  自动雇佣护道者、购买商人物品、死亡复活、关闭打赏弹窗
...
(function () {
  ...
  function loadConfig() {
    ...
    if (saved) {
      try { return typeof saved === 'string' ? JSON.parse(saved) : saved; } catch (e) { /* fallback */ }
    }
  ...
  function createPanel() {
    ...
    document.getElementById('monitor-toggle').addEventListener('click', (e) => {
      ...
      if (window.__monitorRunning) {
        ...
        hiring = false;
        shopping = false;
        if (window.__monitorInterval) { clearInterval(window.__monitorInterval); window.__monitorInterval = null; }
  ...
  function clickHireProtector() {
    ...
    for (const btn of btns) {
      if (btn.textContent.trim() === '雇佣护道') { btn.click(); return; }
    }
  }
  ...
  function dismissTipDialog() {
    ...
    for (const btn of btns) {
      const t = btn.textContent.trim();
      if (t === '取 消' || t === '取消') { btn.click(); return true; }
    }
    return false;
  }
  ...
  async function handleMerchant() {
    ...
    try {
      ...
      if (items.length === 0) { shopping = false; return; }
      ...
      if (!bought) {
        for (const quality of mcfg.stonePriority) {
          const stone = allItems.find(i =>
            i.name.includes(quality) && mcfg.stoneKeywords.some(kw => i.name.includes(kw))
          );
          if (stone) {
  ...
  async function tryEscape(maxAttempts = 5) {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (overlay) {
        const btns = overlay.querySelectorAll('button');
        for (const btn of btns) {
          if (btn.textContent.trim() === '逃跑') { btn.click(); clicked = true; break; }
        }
      }
  ...
  async function hireProtector() {
    ...
    hiring = true;
    const now = Date.now();
    if (now - lastEncounterTime < 10000) { hiring = false; return; }
    ...
    try {
      ...
      const overlay = document.getElementById('encounterOverlay');
      if (!overlay) { log('未找到遭遇界面'); hiring = false; return; }

      // 根据设置决定：优先雇佣，无空闲时迎战或逃跑
      const overlayBtns = overlay.querySelectorAll('button');
      let step1 = false;
      for (const btn of overlayBtns) {
        if (btn.textContent.trim() === '雇佣护道') { btn.click(); step1 = true; break; }
      }
      if (!step1) { log('未找到雇佣护道按钮'); hiring = false; return; }
      log('已点击雇佣护道');
      ...
      if (loaded === 'timeout') { log('护道者列表加载超时'); hiring = false; return; }

      const hired = await findAndHireProtector(1);
      if (!hired) { log('雇佣失败，无合适人选'); hiring = false; return; }
  ...
  async function handleDeath() {
    ...
    for (const btn of iconBtns) {
      if (btn.textContent.includes('地图')) { btn.click(); break; }
    }
    await sleep(300);
    ...
    for (const btn of allBtns) {
      if (btn.textContent.includes('神行符')) { btn.click(); break; }
    }
    await sleep(300);
